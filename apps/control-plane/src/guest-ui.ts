...
(() => {
  ...
  const historyCard = (item) => '<div class="history-item"><span class="meta"><span class="title">'+esc(item.title||item.videoId)+'</span><span class="sub">'+esc(item.channel||'')+'</span></span><span class="sub">'+(item.reason==='ended'?'finished':item.reason)+'</span></div>';
  let lastStatus='';
  const renderStatus = (status) => {const current=status.current;$('now-playing').innerHTML=current?'<b>'+esc(current.title||current.videoId)+'</b><span>'+esc(current.channel||'Playing now')+(current.duration?' · '+esc(current.duration):'')+'</span>':'<b>Nothing playing</b><span>Search for a song</span>';const queue=status.queue||[];const history=status.history||[];queueLength=queue.length;$('queue-list').innerHTML=queue.length?queue.map((item,index)=>queueCard(item,index,queue.length)).join(''):'<div class="queue-empty">The queue is empty. Add a song from Search.</div>';$('history-list').innerHTML=history.length?history.map(historyCard).join(''):'<div class="queue-empty">Songs you finish or skip will appear here.</div>';$('history-count').textContent=history.length+(history.length===1?' song':' songs');$('queue-count').textContent=queue.length+(queue.length===1?' song':' songs');$('queue-badge').textContent=String(queue.length);$('queue-badge').hidden=!queue.length;};
