...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .spinner { width:1.1rem; height:1.1rem; border:2px solid #3f3f3f; border-top-color:var(--accent); border-radius:50%; animation:spin .8s linear infinite; display:inline-block; align-self:center; flex:0 0 auto; }
  ...
  .suggest-label { padding:.6rem .9rem .15rem; color:var(--muted); font-size:.68rem; font-weight:700; text-transform:uppercase; letter-spacing:.08em; }
  ...
  .result .title { white-space:normal; overflow:visible; text-overflow:clip; overflow-wrap:anywhere; }
  .result .sub { white-space:normal; overflow:visible; text-overflow:clip; overflow-wrap:anywhere; line-height:1.35; }
  .quick-add { width:44px; padding:0; font-size:1.45rem; border-radius:50%; }
  .results.searching .quick-add { opacity:.4; pointer-events:none; }
  ...
  .queued { display:grid; grid-template-columns:2rem minmax(0,1fr); gap:.45rem .55rem; align-items:start; background:var(--surface); padding:.65rem .55rem; border-radius:.7rem; }
  .position { color:var(--muted); text-align:center; font-weight:700; line-height:1.35; }
  .queued .title { white-space:normal; overflow:visible; text-overflow:clip; overflow-wrap:anywhere; }
  .queued .sub { white-space:normal; overflow:visible; text-overflow:clip; overflow-wrap:anywhere; line-height:1.35; }
  .queue-requester { color:#d2d2d2; }
  .queue-actions { grid-column:2; display:grid; grid-template-columns:repeat(5,minmax(44px,1fr)); gap:.4rem; width:100%; }
  .queue-actions button { width:100%; min-width:44px; min-height:44px; padding:0; border-radius:.55rem; }
...
<main>
  <section class="panel active" id="search-panel">
    <div class="section-head"><h2>Find a song</h2><span class="spinner" id="search-spinner" hidden aria-hidden="true"></span><span class="count" id="result-count"></span></div>
...
<dialog id="song-actions" aria-labelledby="song-actions-title"><div class="sheet"><h3 id="song-actions-title">Choose an action</h3><div class="sheet-actions">
  <a id="action-preview" class="secondary" href="https://youtu.be/" target="_blank" rel="noopener">▶ Preview on YouTube</a><button id="action-add" class="primary" type="button">Add to queue</button><button id="action-next" class="secondary" type="button">Play next</button><button id="action-now" class="danger" type="button">Play now</button><button id="action-cancel" class="secondary" type="button">Cancel</button>
</div></div></dialog>
...
(() => {
  ...
  let token = null, guestName = '', selectedSong = null, selectedQueueItem = null, allResults = [], queuedItems = [], visibleResults = 0, queueLength = 0, volume = .75, currentItemId = null, observedPlayback = 'unavailable', toastTimer, suggestSequence = 0, searchSequence = 0;
  ...
  $('search-form').addEventListener('submit',async(event)=>{event.preventDefault();const query=$('karaoke-search').value.trim();if(!query)return;clearTimeout(suggestTimer);suggestSequence++;const generation=++searchSequence;$('search-spinner').hidden=false;$('results').classList.add('searching');const controller=typeof AbortController==='function'?new AbortController():null;const timeout=controller?setTimeout(()=>controller.abort(),12000):null;$('search-suggestions').textContent='';setStatus('search-status','Searching…');try{const response=await api('/search',{method:'POST',body:JSON.stringify({query}),...(controller?{signal:controller.signal}:{})});if(generation!==searchSequence)return;if(response.status===401)return forgetToken('Your party token expired.');if(!response.ok)throw new Error('Search failed');const page=await response.json();if(generation!==searchSequence)return;allResults=page.items||[];$('results').textContent='';visibleResults=0;renderMore();const history=recent().filter(value=>value.toLowerCase()!==query.toLowerCase());try{localStorage.setItem(recentKey,JSON.stringify([query,...history].slice(0,8)));}catch{}setStatus('search-status',allResults.length?allResults.length+' results — ordered by YouTube relevance':'No results found');}catch(error){if(generation!==searchSequence)return;setStatus('search-status',error&&error.name==='AbortError'?'Search timed out — keeping prior results.':(error&&error.message)||'Search failed',true);}finally{if(timeout)clearTimeout(timeout);if(generation===searchSequence){$('search-spinner').hidden=true;$('results').classList.remove('searching');}}});
  ...
  let suggestTimer;
  const suggestHtml = (values) => values.length?'<div class="suggest-label">Suggestions</div>'+values.map(value=>'<button type="button" class="suggestion" data-value="'+esc(value)+'">'+esc(value)+'</button>').join(''):'';
  $('karaoke-search').addEventListener('focus',()=>{if($('karaoke-search').value.trim())return;$('search-suggestions').innerHTML=suggestHtml(recent().slice(0,8));});
  $('karaoke-search').addEventListener('input',()=>{clearTimeout(suggestTimer);const query=$('karaoke-search').value.trim();const sequence=++suggestSequence;if(!query){$('search-suggestions').textContent='';return;}suggestTimer=setTimeout(async()=>{try{const response=await api('/suggest?q='+encodeURIComponent(query));if(!response.ok)throw new Error();const remote=(await response.json()).suggestions||[];if(sequence!==suggestSequence||$('karaoke-search').value.trim()!==query)return;const seen=new Set();const values=[...recent().filter(value=>value.toLowerCase().startsWith(query.toLowerCase())),...remote].filter(value=>{const key=value.toLowerCase();if(seen.has(key))return false;seen.add(key);return true;}).slice(0,8);$('search-suggestions').innerHTML=suggestHtml(values);}catch{if(sequence===suggestSequence)$('search-suggestions').textContent='';}},250);});
  ...
  const openSongActions = (item) => {selectedSong=item;$('song-actions-title').textContent=item.title||item.id||item.videoId;const isQueueItem=!!item.itemId;['action-add','action-next','action-now'].forEach(id=>$(id).hidden=isQueueItem);$('action-preview').setAttribute('href','https://youtu.be/'+esc(item.id||item.videoId));$('song-actions').showModal();};
  const queueSong = async (item,path,label,sourceRow,sourceButton) => {const row=sourceRow||[...document.querySelectorAll('.result')].find(candidate=>candidate.dataset.videoId===item.id);const buttons=[...(row?[...row.querySelectorAll('button')]:[]),...(sourceButton?[sourceButton]:[])];let succeeded=false;buttons.forEach(button=>button.disabled=true);try{const response=await api(path,{method:'POST',body:JSON.stringify(itemPayload(item))});if(response.status===401)return forgetToken('Your party token expired.');if(!response.ok)throw new Error('Request failed');succeeded=true;const verb=label==='Added'?'Added “'+(item.title||item.id)+'” to queue':label+' “'+(item.title||item.id)+'”';showToast(verb);const add=row&&row.querySelector('.quick-add');if(add){add.textContent=label;add.classList.add('done');}lastStatus='';await refresh();}catch(error){showToast('Could not update queue',true);}finally{if(!succeeded)buttons.forEach(button=>button.disabled=false);}};
  $('action-add').addEventListener('click',(event)=>{$('song-actions').close();if(selectedSong)queueSong(selectedSong,'/queue','Added',undefined,event.currentTarget);});
  $('action-next').addEventListener('click',(event)=>{$('song-actions').close();if(selectedSong)queueSong(selectedSong,'/queue/next','Playing next:',undefined,event.currentTarget);});
  ...
  $('play-now-confirm-button').addEventListener('click',(event)=>{$('play-now-confirm').close();if(selectedSong)queueSong(selectedSong,'/queue/play-now','Playing now:',undefined,event.currentTarget);});
  $('queue-play-cancel').addEventListener('click',()=>{$('queue-play-confirm').close();selectedQueueItem=null;});
  $('queue-play-confirm-button').addEventListener('click',(event)=>{const item=selectedQueueItem;selectedQueueItem=null;$('queue-play-confirm').close();if(item)control('/queue/play',{itemId:item.itemId},'Playing “'+(item.title||item.videoId)+'”','queue-status',event.currentTarget);});
  ...
  $('clear-queue-confirm-button').addEventListener('click',(event)=>{$('clear-queue-confirm').close();control('/queue/clear',null,'Queue cleared','queue-status',event.currentTarget);});
  ...
  const queueSource = (item) => [item.channel,item.duration].filter(Boolean).map(esc).join(' · ');
  const queueRequester = (item) => item.requestedBy?'Requested by '+item.requestedBy:'';
  const queueCard = (item,index,total) => '<div class="queued"><span class="position">'+(index+1)+'</span><span class="meta"><span class="title">'+esc(item.title||item.videoId)+'</span><span class="sub queue-source">'+queueSource(item)+'</span><span class="sub queue-requester">'+esc(queueRequester(item))+'</span></span><span class="queue-actions"><button class="primary queue-play" type="button" data-item-id="'+esc(item.itemId)+'" aria-label="Play '+esc(item.title||item.videoId)+' now">▶</button><button class="secondary queue-up" type="button" '+(index===0?'disabled':'')+' data-item-id="'+esc(item.itemId)+'" data-position="'+index+'" aria-label="Move '+esc(item.title||item.videoId)+' up">↑</button><button class="secondary queue-down" type="button" '+(index===total-1?'disabled':'')+' data-item-id="'+esc(item.itemId)+'" data-position="'+index+'" aria-label="Move '+esc(item.title||item.videoId)+' down">↓</button><button class="secondary queue-remove" type="button" data-item-id="'+esc(item.itemId)+'" aria-label="Remove '+esc(item.title||item.videoId)+'">✕</button><button class="secondary queue-more" type="button" data-item-id="'+esc(item.itemId)+'" aria-label="More actions for '+esc(item.title||item.videoId)+'">⋯</button></span></div>';
