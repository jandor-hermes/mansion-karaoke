...
<div class="auth-gate" id="auth-gate">
  <form class="token-row" id="token-form">
    <h1>🎤 Join ${room}</h1><p>Enter the party token from the host.</p>
    <input id="token-input" type="password" autocomplete="current-password" placeholder="Party token" required>
...
<header>
  <div class="topline"><strong>🎤 Karaoke</strong><span class="room">${room}</span><button id="change-token" class="secondary change-token" type="button">Change token</button></div>
...
<main>
  ...
  <section class="panel" id="queue-panel">
    <div class="section-head"><h2>Up next</h2><span class="count" id="queue-count"></span></div>
...
(() => {
  ...
  let token = null, selectedSong = null, allResults = [], visibleResults = 0, queueLength = 0, volume = .75, playing = true, toastTimer, suggestSequence = 0;
  try { token = localStorage.getItem(tokenKey) || localStorage.getItem(legacyTokenKey); } catch {}
  ...
  const forgetToken = (message) => { token=null; try{localStorage.removeItem(tokenKey);localStorage.removeItem(legacyTokenKey);}catch{} $('controller-shell').hidden=true; $('auth-gate').hidden=false; setStatus('token-status',message||'',!!message); setTimeout(()=>$('token-input').focus(),0); };
  const validateToken = async () => { try { const response=await api('/status'); if(!response.ok) throw new Error(response.status===401?'That party token was not accepted.':'Could not connect.'); $('auth-gate').hidden=true; $('controller-shell').hidden=false; try{localStorage.setItem(tokenKey,token);localStorage.removeItem(legacyTokenKey);}catch{} renderStatus(await response.json()); return true; } catch(error){ forgetToken(error && error.message ? error.message : 'Could not connect.'); return false; } };
  $('token-form').addEventListener('submit',async(event)=>{event.preventDefault(); const value=$('token-input').value.trim(); if(!value){setStatus('token-status','Enter a party token.',true);return;} token=value; setStatus('token-status','Joining…'); $('token-save').disabled=true; await validateToken(); $('token-save').disabled=false;});
  $('change-token').addEventListener('click',()=>forgetToken('Enter a different party token.'));
  ...
  $('search-form').addEventListener('submit',async(event)=>{event.preventDefault();const query=$('karaoke-search').value.trim();if(!query)return;$('search-suggestions').textContent='';setStatus('search-status','Searching…');try{const response=await api('/search',{method:'POST',body:JSON.stringify({query})});if(response.status===401)return forgetToken('Your party token expired.');if(!response.ok)throw new Error('Search failed');const page=await response.json();allResults=page.items||[];visibleResults=0;renderMore();const history=recent().filter(value=>value.toLowerCase()!==query.toLowerCase());try{localStorage.setItem(recentKey,JSON.stringify([query,...history].slice(0,8)));}catch{}setStatus('search-status',allResults.length?allResults.length+' results':'No results found');}catch(error){allResults=[];$('results').textContent='';$('load-more').hidden=true;setStatus('search-status',error.message||'Search failed',true);}});
  ...
  const itemPayload = (item) => ({itemId:(((globalThis.crypto&&typeof globalThis.crypto.randomUUID==='function')&&globalThis.crypto.randomUUID())||Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,10)),videoId:item.id,title:item.title||item.id,...(item.channel?{channel:item.channel}:{}),...(item.duration?{duration:item.duration}:{}),...(item.thumbnail?{thumbnail:item.thumbnail}:{})});
  ...
  if(token)validateToken(); else forgetToken();
