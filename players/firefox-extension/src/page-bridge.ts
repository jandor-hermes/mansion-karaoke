import { parseLoadVideoCommand, type LoadVideoCommand, type LoadVideoResult, YOUTUBE_VIDEO_ID } from './load-video';

export type YouTubePageAdapter = {
    dispatchNavigate(videoId: string, position: number): boolean;
...
export async function loadVideoInPage(raw: unknown, adapter: YouTubePageAdapter, attempts = 20): Promise<LoadVideoResult> {
...
function installPageBridge(): void {
    ...
    node.addEventListener(`karaoke-load-request-${channel}`, () => {
        void (async () => {
            try {
                const envelope = JSON.parse(node.dataset.karaokeRequest ?? 'null') as { requestId?: unknown; videoId?: unknown; position?: unknown } | null;
                ...
                const command: LoadVideoCommand = { type: 'loadVideo', videoId: envelope.videoId, position: Number(envelope.position) };
            ...
            } catch { /* malformed page request has no authority */ }
        })();
    });
}
